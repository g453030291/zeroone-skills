...
文案 + 提示重试。跑完告诉用户报告在哪，如果 `share_url` 有效，一并发给用户；如果失败，
用 stderr 里的人话原因告诉用户（比如"token 过期了，邮件联系 gems9232@foxmail.com"）。
如果 `finalize` 返回的 `alerts` 非空，也要主动提醒用户"最近的采集出现了问题"。

如果用户想看历史报告，把首页 `data/reports/dashboard.html` 的路径发给他即可——那是一份
可以来回打开的日期目录，不需要你每次都重新生成。
