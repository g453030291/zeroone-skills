...
4. **建立自动化**：这一步不是运行某个脚本，而是你（Agent）直接使用当前宿主平台提供的
   ...
   - **采集任务**：一天触发 `config.json` 的 `harvest_hours` 里配的那几个时刻（默认
     ...
     这一步不需要语义推理，是纯脚本操作，唤醒后几秒就能跑完。把下面的通知规则一并写进
     任务 prompt：**退出码为 0 时保持静默**，不要发送成功消息；返回非 0 或发生异常时，
     只发送一条简短通知：
     `monitor-anything 采集失败｜时间：<触发时间>｜异常：<失败阶段 + 人话原因>`。
     不要附完整堆栈、token 或重复重试日志。
   - **报告任务**：一天触发一次，时刻取 `config.json` 的 `report_time`（默认 `08:00`）。
     ...
     把下面的通知规则一并写进任务 prompt：**无论成功或失败，结束时都只发送一条简短
     通知**，不要把逐步进度当成通知反复发送。通知必须包含日期、报告入口、分享链接和异常
     摘要，格式固定为：

     - 成功：`monitor-anything 日报已生成｜日期：<date>｜报告：<dashboard.html；今日 date.html>｜分享：<share_url 或未生成 + 原因>｜异常：<无或 alerts 摘要>`
     - 失败：`monitor-anything 日报失败｜日期：<date>｜报告：<已生成的入口，尚未生成则写“未生成”>｜分享：<已有 share_url，尚未生成则写“未生成”>｜异常：<失败阶段 + 人话原因>`

     分享上传失败不推翻已经成功生成的本地报告：此时使用成功格式，在“分享”和“异常”中
     说明上传失败。报告流程真正失败时，发送失败通知后仍保留任务的失败状态，不要假装完成。

   这里的“通知”是宿主平台中这次自动化任务的最终用户消息，不要求额外接入邮件、群机器人
   或移动端推送。上述规则只约束本次 Setup 新创建的任务；不要扫描或修改用户已有的自动化。
...
按提示里列出的 monitor id，从 `candidates` 开始重跑它们，再 finalize 一次。如果这是定时
报告任务且重跑后仍失败，按 Setup 中的失败格式发送一次通知。
...
定时报告任务把这些内容压缩进最终通知的“异常”字段；没有异常时明确写“无”。手动运行时
仍按正常交付方式告诉用户，不必套用自动化通知模板。
