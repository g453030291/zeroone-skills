...
`cross_source` / `source_count` 由脚本算好，口径是**独立信源**（按域名判定，公众号、
小红书这类共享平台域名下按账号判定），不是渠道类别——量子位和机器之心算两个信源，
纽约时报自有渠道与补充检索搜回的同一家稿子算一个。写 `summary` 时如果要提"跨了几个源"，
直接用 `source_count`，不要自己按 `source_type` 重新数。
...
- `summary`：2~4 句摘要，说清楚「发生了什么」，如果是跨源聚类可以简单点出不同信源的角度差异
