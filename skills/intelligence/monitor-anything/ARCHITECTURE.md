...
两条任务的通知强度不同：高频采集成功时静默、失败才提醒；每天一次的报告任务无论成功失败
都发送一条最终消息，固定汇总日期、报告入口、分享链接和异常。通知依赖宿主平台的任务消息，
不是重新引入 §16 已移除的邮件/webhook 输出层。Setup 只把规则写进新任务，不迁移既有任务。
...
需要用户做任何事。这个接口不需要携带鉴权信息。同一客户端 IP 在滚动 30 天内最多申请
10 个临时 Token，达到上限返回 429；IP 只用于申请限额和调用记录，Token 创建后不绑定使用
IP，可以在其他网络环境继续使用。客户端不发送 `X-Forwarded-For`，可信代理识别和
`TRUSTED_PROXY_NETWORKS` 配置属于服务端职责。

Skill 仍把"没有 token"和"已有 token 但过期了"作为两条不同的产品路径：
...
为什么过期后不直接再自动申请一个新的试用 token：服务端的 10 个额度用于首次安装、配置丢失
等自助场景，并不等于授权客户端把临时 Token 当作自动续期机制。让"继续用"这个决定回到人工
审核的邮件流程，是本 Skill 保留的产品边界，不是接口限制，所以客户端不会在 401 后自动申请。
申请时如果收到 429，`setup.py` 会直接说明最近 30 天额度已满，不会误报成网络问题或重复重试。
...
## 24. 自动化通知：采集只报失败，报告始终有结果

自动化原先只定义“运行什么”，没有定义“什么时候值得打断用户”。宿主平台即使保存了任务
结果，也不等于用户收到了有意义的提醒。现在创建任务时把通知策略直接写进任务 prompt：

- 采集一天运行多次，成功消息没有决策价值，因此退出码为 0 时静默；失败时只报触发时间、
  失败阶段和人话原因。
- 报告一天只有一次，是用户真正消费的产物；成功和失败都只发一条最终消息，固定包含日期、
  报告入口、分享链接和异常摘要。
- `share.py upload` 失败仍沿用 §20 的降级语义：本地报告已经生成就算报告成功，在通知里把
  分享链接写成未生成并说明异常，不把整次报告误报为失败。

这里的通知只指宿主平台里的自动化任务最终消息。它不增加脚本依赖，不恢复邮件或群机器人
dispatcher，也不要求移动端推送。为了保持变更边界清楚，只影响此后 Setup 新建的任务。
